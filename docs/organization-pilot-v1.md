...
N≥2g; g≥3; N divisible by g. Default development N=32,g=8,R=3. Seed deterministically permutes group membership using SHA-256 ordering; realized memberships and every round's edges are stored. L has isolated groups; X is strongly connected because local successor and group successor generate every position. Global connectedness is not fast mixing: the X group ring has long diameter at 10K. With only three rounds, at most six initial agents can be in a final X agent's causal neighborhood, including itself; content retention may be worse. Longer rounds or rewired shortcuts need separate tests. These are particular sparse graphs, not all possible local/global architectures. The final source round has no messages. The compiler rejects any manifest exceeding the existing 2 MiB event payload limit; higher N×R may require separately hashed graph artifacts before execution.
