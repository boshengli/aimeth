...
(async()=>{
 ...
 const name='m2-1-cluster-pilot-v2';
 ...
 try{
  ...
  for(const width of [1440,768,390]){
   ...
   assert(text.includes('14 个因调度器提前退出，7 个因 8 次输出截断停止。'));
  ...
  for(const href of hrefs.filter(h=>h&&!h.startsWith('http'))){
   ...
   else if(href!=='../cluster-report-v2-manifest.json')assert(fs.existsSync(path.resolve(path.dirname(input),href)),`Missing ${href}`);
  }
